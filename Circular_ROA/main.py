...
# This file serves to implement the main code necessary to integrate the Yuan-Li PDE using the Physics Informed Neural Network (PINN) framework to determine the region of attraction (ROA) associated with a circular ROA.
...
save_path = r'./Circular_ROA/Save'                                                                    # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./Circular_ROA/Load'                                                                    # [-] Relative path to the directory from which to load network data.
...
# classification_noise_percentage = torch.tensor( 2e-5, dtype = torch.float32, device = device )      # [%] The classification point noise magnitude represented as a percentage of the domain volume.  This parameter conveniently scales the noise magnitude of the classification points as the dimension of the problem is adjusted.
...
# # hidden_layer_widths = torch.tensor( 100, dtype = torch.uint8, device = device )              # [#] Hidden layer widths.
# # hidden_layer_widths = torch.tensor( 200, dtype = torch.uint8, device = device )              # [#] Hidden layer widths.
# # hidden_layer_widths = torch.tensor( 500, dtype = torch.uint8, device = device )              # [#] Hidden layer widths.
# # hidden_layer_widths = torch.tensor( 1000, dtype = torch.uint8, device = device )              # [#] Hidden layer widths.
# # hidden_layer_widths = torch.tensor( 2000, dtype = torch.uint8, device = device )              # [#] Hidden layer widths.
# # hidden_layer_widths = torch.tensor( 5000, dtype = torch.uint8, device = device )              # [#] Hidden layer widths.
...
# # num_training_data = torch.tensor( int( 500e3 ), dtype = torch.int32, device = device )      # [#] Number of training data points.
# # num_testing_data = torch.tensor( int( 100e3 ), dtype = torch.int32, device = device )        # [#] Number of testing data points.
...
# # learning_rate = torch.tensor( 1e-3, dtype = torch.float32, device = device )                # [-] Learning rate.
...
# Store the loss coefficients.
# c_IC = torch.tensor( 1, dtype = torch.float32, device = device )                          # [-] Initial condition loss weight.
# c_BC = torch.tensor( 1, dtype = torch.float32, device = device )                          # [-] Boundary condition loss weight.
# c_residual = torch.tensor( 8.125e-2, dtype = torch.float32, device = device )                    # [-] Residual loss weight.
# c_variational = torch.tensor( 8.125e-2, dtype = torch.float32, device = device )                 # [-] Variational loss weight.
# c_monotonicity = torch.tensor( 2e1, dtype = torch.float32, device = device )               # [-] Monotonicity loss weight.

c_IC = torch.tensor( 1, dtype = torch.float32, device = device )                          # [-] Initial condition loss weight.
c_BC = torch.tensor( 1, dtype = torch.float32, device = device )                          # [-] Boundary condition loss weight.
c_residual = torch.tensor( 8.1875e-2, dtype = torch.float32, device = device )                    # [-] Residual loss weight.
c_variational = torch.tensor( 8.1875e-2, dtype = torch.float32, device = device )                 # [-] Variational loss weight.
c_monotonicity = torch.tensor( 2e1, dtype = torch.float32, device = device )               # [-] Monotonicity loss weight.
