...
#%% ---------------------------------------- NON-SPIKING SPIRAL ROA PINN EXAMPLE MAIN SCRIPT ----------------------------------------

# This file serves to implement the main code necessary to integrate the Yuan-Li PDE for a dynamical system with a spiral ROA using the Non-Spiking Physics Informed Neural Network (PINN) framework.
...
save_path = r'./ann/spiral_roa/save'                                                                    # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./ann/spiral_roa/load'                                                                    # [-] Relative path to the directory from which to load network data.
...
spatial_domain = torch.tensor( [ [ -1, 1 ], [ -1, 1 ] ], dtype = torch.float32, device = device ).T

# Define the initial condition parameters.
R0 = torch.tensor( 0.25, dtype = torch.float32, device = device )
...
flow_function1 = lambda s: torch.unsqueeze( -s[ :, 2 ], dim = 1 )
flow_function2 = lambda s: torch.unsqueeze( -( -s[ :, 1 ] + s[ :, 2 ]*( 1 - 3*s[ :, 1 ]**2 - 2*s[ :, 2 ]**2 ) ), dim = 1 )
...
pde_name = 'Yuan-Li PDE: Spiral ROA'                                                                                                            # [-] PDE name.
...
c_variational = torch.tensor( 1, dtype = torch.float32, device = device )                 # [-] Variational loss weight.
