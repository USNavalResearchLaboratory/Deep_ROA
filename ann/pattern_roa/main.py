...
#%% ---------------------------------------- NON-SPIKING PATTERN ROA PINN EXAMPLE MAIN SCRIPT ----------------------------------------

# This file serves to implement the main code necessary to integrate the Yuan-Li PDE for a dynamical system with a patterned ROA using the Non-Spiking Physics Informed Neural Network (PINN) framework.
...
save_path = r'./ann/pattern_roa/save'                                                                    # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./ann/pattern_roa/load'                                                                    # [-] Relative path to the directory from which to load network data.
...
temporal_domain = torch.tensor( [ 0, 10 ], dtype = torch.float32, device = device )
spatial_domain = torch.tensor( [ [ -2*math.pi, 2*math.pi ], [ -2*math.pi, 2*math.pi ] ], dtype = torch.float32, device = device ).T

# Define the initial condition parameters.
R0 = torch.tensor( 1.00, dtype = torch.float32, device = device )
...
flow_function2 = lambda s: torch.unsqueeze( -0.5*torch.sin( s[ :, 2 ] ) - 0.5*torch.sin( s[ :, 2 ] - s[ :, 1 ] ) + 0.05, dim = 1 )
...
# c_residual = torch.tensor( 1, dtype = torch.float32, device = device )                      # [-] Residual loss weight.
# c_variational = torch.tensor( 1e-1, dtype = torch.float32, device = device )                # [-] Variational loss weight.
# c_monotonicity = torch.tensor( 1, dtype = torch.float32, device = device )                  # [-] Monotonicity loss weight.

# Store the loss coefficients.
c_IC = torch.tensor( 0.49937617, dtype = torch.float32, device = device )                            # [-] Initial condition loss weight.
c_BC = torch.tensor( 0.49937617, dtype = torch.float32, device = device )                            # [-] Boundary condition loss weight.
c_residual = torch.tensor( 0.49937617, dtype = torch.float32, device = device )                      # [-] Residual loss weight.
c_variational = torch.tensor( 0.04993762, dtype = torch.float32, device = device )                # [-] Variational loss weight.
c_monotonicity = torch.tensor( 0.49937617, dtype = torch.float32, device = device )                  # [-] Monotonicity loss weight.
