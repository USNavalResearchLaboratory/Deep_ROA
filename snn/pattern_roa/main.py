...
#%% ---------------------------------------- SPIKING PATTERNED ROA PINN EXAMPLE MAIN SCRIPT ----------------------------------------

# This file serves to implement the main code necessary to integrate the Yuan-Li PDE for a dynamical system with a patterned roa using the spiking Physics Informed Neural Network (PINN) framework.
...
save_path = r'./snn/pattern_roa/save'                                 # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./snn/pattern_roa/load'                                 # [-] Relative path to the directory from which to load network data.
...
spatial_domain = torch.tensor( [ [ -2*math.pi, 2*math.pi ], [ -2*math.pi, 2*math.pi ] ], dtype = torch.float32, device = device ).T         # [-] Spatial domain of the underlying dynamical system.

# Define the initial condition parameters.
R0 = torch.tensor( 0.50, dtype = torch.float32, device = device )                                              # [-] Initial condition radius.
...
P0_shift = torch.tensor( [ 0.0, 0.0 ], dtype = torch.float32, device = device )                 # [-] Initial condition input offset.
z0_shift = -A0/2                                                                                            # [-] Initial condition output offset.

# Define the flow functions.
flow_function1 = lambda s: torch.unsqueeze( torch.unsqueeze( -torch.sin( s[ :, 1, -1 ] ) - 0.5*torch.sin( s[ :, 1, -1 ] - s[ :, 2, -1 ] ) + 0.01, dim = 1 ), dim = 2 )                  # [-] Flow function associated with the first state of the underlying dynamical system.
flow_function2 = lambda s: torch.unsqueeze( torch.unsqueeze( -0.5*torch.sin( s[ :, 2, -1 ] ) - 0.5*torch.sin( s[ :, 2, -1 ] - s[ :, 1, -1 ] ) + 0.05, dim = 1 ), dim = 2 )              # [-] Flow function associated with the second state of the underlying dynamical system.
...
pde_name = 'Yuan-Li PDE: Pattern ROA'
...
# residual_batch_size = torch.tensor( 64, dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( 256, dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( 500, dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( int( 1e3 ), dtype = torch.int32, device = device )                      # This works for variational loss integration order 2.
# residual_batch_size = torch.tensor( int( 5e3 ), dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( int( 7.5e3 ), dtype = torch.int32, device = device )                 # This works for variational loss integration order 1.
...
# residual_batch_size = torch.tensor( int( 50e3 ), dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( int( 80e3 ), dtype = torch.int32, device = device )            # Worked well when only using residual (memory error when adding variational)
# residual_batch_size = torch.tensor( int( 100e3 ), dtype = torch.int32, device = device )            # Worked well when only using residual (memory error when adding variational)
# residual_batch_size = torch.tensor( int( 125e3 ), dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( int( 150e3 ), dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( int( 200e3 ), dtype = torch.int32, device = device )
# residual_batch_size = torch.tensor( int( 250e3 ), dtype = torch.int32, device = device )
...
# c_monotonicity = torch.tensor( 20.0, dtype = torch.float32, device = device )           # [-] Monotonicity loss weight.

# c_IC = torch.tensor( 1.0, dtype = torch.float32, device = device )                      # [-] Initial condition loss weight.
# c_BC = torch.tensor( 1.0, dtype = torch.float32, device = device )                      # [-] Boundary condition loss weight.
# c_residual = torch.tensor( 1.0, dtype = torch.float32, device = device )                # [-] Residual loss weight.
# c_variational = torch.tensor( 1.0, dtype = torch.float32, device = device )             # [-] Variational loss weight.
# c_monotonicity = torch.tensor( 1.0, dtype = torch.float32, device = device )           # [-] Monotonicity loss weight.
