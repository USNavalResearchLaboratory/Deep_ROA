...
#%% ---------------------------------------- SPIKING SPIRAL ROA PINN EXAMPLE MAIN SCRIPT ----------------------------------------

# This file serves to implement the main code necessary to integrate the Yuan-Li PDE for a dynamical system with a spiral ROA using the Spiking Physics Informed Neural Network (PINN) framework.
...
save_path = r'./snn/spiral_roa/save'                                                                # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./snn/spiral_roa/load'                                                                # [-] Relative path to the directory from which to load network data.
...
classification_tfinal = torch.tensor( 10.0, dtype = torch.float32, device = device )                # [s] The classification simulation duration used to forecast classification points.
...
temporal_domain = torch.tensor( [ 0.0, 10.0 ], dtype = torch.float32, device = device )                         # [-] Temporal domain of the underlying dynamical system.
spatial_domain = torch.tensor( [ [ -1.0, 1.0 ], [ -1.0, 1.0 ] ], dtype = torch.float32, device = device ).T     # [-] Spatial domain of the underlying dynamical system.
...
flow_function1 = lambda s: torch.unsqueeze( torch.unsqueeze( -s[ :, 2, -1 ], dim = 1 ), dim = 2 )
flow_function2 = lambda s: torch.unsqueeze( torch.unsqueeze( -( -s[ :, 1, -1 ] + s[ :, 2, -1 ]*( 1 - 3*s[ :, 1, -1 ]**2 - 2*s[ :, 2, -1 ]**2 ) ), dim = 1 ), dim = 2 )
...
num_timesteps = torch.tensor( 10, dtype = torch.int16, device = device )
...
# num_epochs = torch.tensor( int( 1e1 ), dtype = torch.int32, device = device )                         # [#] Number of training epochs to perform.
...
# Ensure that the number of noisy samples per level set point is updated properly.
