...
class neural_network_class( torch.nn.Module ):
    ...
    def __init__( self, neuron_parameters, synapse_parameters, layers, activation_string, learning_rate, residual_batch_size, num_epochs, residual_function, residual_code, temporal_code, training_data, testing_data, plotting_data, dimension_labels, element_computation_option, batch_print_frequency, epoch_print_frequency, c_IC, c_BC, c_residual, c_variational, c_monotonicity, newton_tolerance, newton_max_iterations, exploration_radius_spatial, exploration_radius_spatiotemporal, num_exploration_points, unique_tolerance_spatial, unique_tolerance_spatiotemporal, classification_noise_magnitude_spatial, classification_noise_magnitude_spatiotemporal, device = 'cpu', verbose_flag = False ):
        ...
        self.forward_stack = self.initialize_forward_stack( layers = self.layers, neuron_parameters = self.neuron_parameters, synapse_parameters = self.synapse_parameters )
    ...
    # Implement a function to preprocess the synapse parameters.
    def preprocess_synapse_parameters( self, synapse_parameters = None ):

        # Determine whether to use the default synapse parameters.
        if synapse_parameters is None:          # If the synapse parameters were not provided...

            # Set the synapse parameters to be the default value.
            synapse_parameters = self.synapse_parameters

        # Return the synapse parameters.
        return synapse_parameters


    # Implement a function to preprocess the neuron parameters.
    def preprocess_neuron_parameters( self, neuron_parameters = None ):

        # Determine whether to use the default neuron parameters.
        if neuron_parameters is None:           # If the neuron parameters were not provided...

            # Set the neuron parameters to be the default value.
    ...
    def preprocess_network_classifications( self, network_classifications ):
        ...
        # Preprocess the network classifications.
        if network_classifications is None:             # If the network classifications was not provided...

            # Set the network classifications to be the stored value.
            network_classifications = self.network_classifications

        # Return the network classifications.
        return network_classifications


    # Implement a function to preprocess the actual classifications.
    def preprocess_actual_classifications( self, actual_classifications ):

        # Preprocess the actual classifications.
        if actual_classifications is None:             # If the actual classifications was not provided...

            # Set the actual classifications to be the stored value.
            actual_classifications = self.actual_classifications

        # Return the actual classifications.
        return actual_classifications


    # Implement a function to simply preprocess the classification data.
    def preprocess_classification_data_simple( self, classification_data ):

        # Preprocess the classification data.
        if classification_data is None:             # If the classification data was not provided...

            # Set the classification data to be the stored value.
    ...
    # Implement a function to simply preprocess the classification forecast data.
    def preprocess_classification_data_forecast_simple( self, classification_data_forecast ):

        # Preprocess the classification forecast data.
        if classification_data_forecast is None:        # If the classification forecast data was not provided...

            # Set the classification data forecast to be the stored value.
            classification_data_forecast = self.classification_data_forecast
    ...
    def setup_classification_data( self, classification_data, classification_data_forecast, actual_classifications, network_classifications ):
        ...
        # Preprocess the network classification data.
        network_classifications = self.preprocess_network_classifications( network_classifications )

        # Preprocess the actual classification data.
        actual_classifications = self.preprocess_actual_classifications( actual_classifications )

        # Preprocess the classification data (simply).
        classification_data = self.preprocess_classification_data_simple( classification_data )

        # Preprocess the classification data forecast (simply).
        classification_data_forecast = self.preprocess_classification_data_forecast_simple( classification_data_forecast )

        # Return the classification data information.
        return classification_data, classification_data_forecast, actual_classifications, network_classifications
    ...
    def initialize_forward_stack( self, layers = None, neuron_parameters = None, synapse_parameters = None ):
        ...
        # Preprocess the synapse parameters.
        synapse_parameters = self.preprocess_synapse_parameters( synapse_parameters )

        # Preprocess the neuron parameters.
        neuron_parameters = self.preprocess_neuron_parameters( neuron_parameters )
        ...
        num_layers = torch.tensor( len( layers ), dtype = torch.int8, device = self.device )

        # Create each of the layers.
        for k in range( num_layers - 2 ):           # Iterate through each of the layers...

            # Create the synapse layer associated with this layer.
            forward_stack.append( slayer.synapse.Dense( layers[ k ].item(  ), layers[ k + 1 ].item(  ), weight_scale = synapse_parameters[ "gain" ].item(  ) ).to( self.device ) )

            # Create the neuron layer associated with this layer.
            forward_stack.append( slayer.neuron.cuba.Neuron( threshold = neuron_parameters[ "threshold" ].item(  ), current_decay = neuron_parameters[ "current_decay" ].item(  ), voltage_decay = neuron_parameters[ "voltage_decay" ].item(  ), persistent_state = self.neuron_parameters[ "persistent_state" ], requires_grad = self.neuron_parameters[ "requires_grad" ] ).to( self.device ) )

        # Create the final synapse layer.
        forward_stack.append( slayer.synapse.Dense( layers[ -2 ].item(  ), layers[ -1 ].item(  ), weight_scale = synapse_parameters[ "gain" ].item(  ) ).to( self.device ) )
    ...
    def predict_over_grid( self, network_input_grid ):
        ...
        # Determine if the grid is temporal.
        temporal_grid_flag = self.tensor_utilities.is_expanded_grid_temporal( network_input_grid )

        # Define the subgrid.
        if temporal_grid_flag:          # If the grid is temporal...

            # Set the subgrid.
            subgrid = network_input_grid.shape[ :-2 ]

        else:                           # Otherwise...
            
            # Set the subgrid.
            subgrid = network_input_grid.shape[ :-1 ]
        ...
        network_output_grid = self.tensor_utilities.expand_grid( network_output, subgrid )
    ...
    def variation( self, xs_integration_points_batch = None, G_basis_values_batch = None, W_integration_weights_batch = None, sigma_jacobian_batch = None, derivative_required_for_residual = None, residual_code = None ):
        ...
        num_timesteps = xs_integration_points_batch.shape[ 3 ]
    ...
    def compute_classification_loss( self, pde, classification_data = None, num_spatial_dimensions = None, domain = None, plot_time = None, level = None, level_set_guesses = None, num_guesses = None, newton_tolerance = None, newton_max_iterations = None, exploration_radius = None, num_exploration_points = None, unique_tolerance = None, classification_noise_magnitude = None, num_noisy_samples_per_level_set_point = None, domain_subset_type = 'spatial', tspan = None, dt = None, save_data = True ):
        ...
        if classification_data.numel(  ) != 0:                  # If their is classification data...
            ...
            # Set the forecast classification data to be the classification loss.
            classification_data_forecast = torch.clone( classification_data )

            # Set the network classifications to be none.
            network_classifications = None

            # Set the actual classifications to be none.
            actual_classifications = None

            # Set the classification loss to 100.
            classification_loss = torch.tensor( 100, dtype = torch.float32, device = self.device )

        # Determine whether to store the network classifications, actual classifications, and classification loss.
        if save_data:               # If we want to save the classification information...

            # Store the classification information.
            self.classification_data_forecast = classification_data_forecast
    ...
    def train_epoch( self, training_data = None, num_batches = None, derivative_required_for_residual = None, residual_code = None, print_batch_info_flag = True ):
        ...
        for k in range( num_batches ):                  # Iterate through each of the batches...
            ...
            batch_losses[ k ], batch_losses_ic[ k ], batch_losses_bc[ k ], batch_losses_residual[ k ], batch_losses_variational[ k ], batch_losses_monotonicity[ k ], batch_durations[ k ] = self.train_batch( training_data, derivative_required_for_residual, residual_code )
    ...
    def level_function_at_time( self, spatial_points, time, num_timesteps, domain ):
        ...
        masks = torch.unsqueeze( torch.unsqueeze( domain.contains( spatial_points, 'spatial' ), dim = 1 ), dim = 2 )

        # Compute the network prediction at each point.
        level_values_network = self.forward( torch.tile( torch.unsqueeze( torch.cat( ( torch.repeat_interleave( torch.unsqueeze( time, dim = 0 ), num_spatial_points, dim = 0 ), spatial_points ), dim = 1 ), dim = 2 ), [ 1, 1, num_timesteps ] ) )

        # Compute the distance to each point.
        level_values_distance = torch.tile( torch.unsqueeze( torch.linalg.norm( spatial_points, ord = 2, dim = 1, keepdim = True )**2, dim = 2 ), [ 1, 1, num_timesteps ] )
    ...
    def generate_level_set_at_time( self, num_spatial_dimensions, num_timesteps, domain, plot_time, level = None, level_set_guess = None, newton_tolerance = None, newton_max_iterations = None, exploration_radius = None, num_exploration_points = None, unique_tolerance = None, domain_subset_type = 'spatial' ):
    ...
    def plot_training_results( self, training_epochs = None, training_losses = None, ic_training_losses = None, bc_training_losses = None, residual_training_losses = None, variational_training_losses = None, monotonicity_training_losses = None, testing_epochs = None, testing_losses = None, ic_testing_losses = None, bc_testing_losses = None, residual_testing_losses = None, variational_testing_losses = None, monotonicity_testing_losses = None, c_IC = None, c_BC = None, c_residual = None, c_variational = None, c_monotonicity = None, save_directory = r'.', show_plot = False ):
        ...
        plt.plot( training_epochs, training_losses, 'D-', color = 'k', markersize = markersize, label = 'Training' )
        plt.plot( testing_epochs, testing_losses, 'o-', color = 'k', markersize = markersize, label = 'Testing' )
        plt.legend(  )
    ...
    def plot_classifications( self, classification_data = None, classification_data_forecast = None, actual_classifications = None, network_classifications = None, fig = None, dimension_labels = None, save_directory = r'.', as_surface = False, as_stream = False, as_contour = False, show_plot = False ):

        # Setup for plotting the classification data.
        ...
        if torch.numel( classification_data ) != 0:                 # If the classification data exists...
            ...
            # Generate the title string.
            title_string = f'Network Classification Data'

            # Set the level.
            level = torch.tensor( 0, dtype = torch.float32, device = self.device )

            # Compute the values associated with the classification points.
            classification_values = self.forward( classification_data )
            classification_values = torch.zeros_like( classification_values )

            # Compute the values associated with the classification forecast points.
            classification_values_forecast = self.forward( classification_data_forecast )
            classification_values_forecast = torch.zeros_like( classification_values_forecast )
            ...
            # Retrieve the correct classification points.
            ...
            if ( classification_data_correct_input.numel(  ) != 0 ) and ( classification_data_correct_output.numel(  ) != 0 ):            # If there is correct classification data to plot...
                ...
                # Plot the correct classification data.
                figs, axes = self.plotting_utilities.plot( classification_data_correct_input, classification_data_correct_output, projection_dimensions, projection_values, level, fig[ 0 ], input_labels, title_string, save_directory, as_surface, as_stream, as_contour, show_plot, D1_style_correct )
            
            # Determine whether there is incorrect classification data to plot.
            if ( classification_data_incorrect_input.numel(  ) != 0 ) and ( classification_data_incorrect_output.numel(  ) != 0 ):                       # If there is incorrect classification data to plot...
