...
class function_utilities_class(  ):
    ...
    def evaluate_function_over_expanded_grid( self, input_data, function ):
        ...
        # Determine whether the input grid is temporal.
        temporal_grid_flag = self.tensor_utilities.is_grid_temporal( input_data )

        # Retrieve the subgrid dimensions.
        if temporal_grid_flag:                  # If this is a temporal grid...

            # Set the subgrid dimensions.
            subgrid_dims = input_data[ ..., 0, 0 ].shape

        else:                                   # If this is a nontemporal grid...

            # Set the subgrid dimensions.
            subgrid_dims = input_data[ ..., 0 ].shape
        ...
        output_data = self.tensor_utilities.expand_grid( output_data_flattened, subgrid_dims )
    ...
    def evaluate_functions( self, input_data, functions, as_tensor = True, temporal_grid_flag = None ):

        # Determine whether the input grid is a temporal grid.
        temporal_grid_flag = self.tensor_utilities.is_grid_temporal( input_data )
