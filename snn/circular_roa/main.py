...
#%% ---------------------------------------- SPIKING CIRCULAR ROA EXAMPLE MAIN SCRIPT ----------------------------------------

# This file serves to implement the main code necessary to integrate the Yuan-Li PDE for a system with a circular roa using a Spiking Physics Informed Neural Network (PINN) framework.
...
save_path = r'./snn/circular_roa/save'                                 # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./snn/circular_roa/load'                                 # [-] Relative path to the directory from which to load network data.
...
# Define the flow function parameters.
mu = torch.tensor( 12, dtype = torch.float32, device = device )
sqrsum = lambda s: s[ :, 1, -1 ]**2 + s[ :, 2, -1 ]**2
comm = lambda s: mu + sqrsum( s ) - sqrsum( s )**2

# Define the flow functions.
flow_function1 = lambda s: torch.unsqueeze( torch.unsqueeze( -( s[ :, 1, -1 ]*comm( s ) - s[ :, 2, -1 ] ), dim = 1 ), dim = 2 )             # [-] Flow function associated with the first state of the underlying dynamical system.
flow_function2 = lambda s: torch.unsqueeze( torch.unsqueeze( -( s[ :, 2, -1 ]*comm( s ) + s[ :, 1, -1 ] ), dim = 1 ), dim = 2 )             # [-] Flow function associated with the second state of the underlying dynamical system.
...
hidden_layer_widths = torch.tensor( int( 2e4 ), dtype = torch.int16, device = device )              # [#] Hidden layer widths.
...
p_initial = torch.tensor( 0.25, dtype = torch.float16, device = device )
p_boundary = torch.tensor( 0.25, dtype = torch.float16, device = device )
p_residual = torch.tensor( 0.5, dtype = torch.float16, device = device )
...
# c_BC = torch.tensor( 5.0, dtype = torch.float32, device = device )
# c_residual = torch.tensor( 0.05, dtype = torch.float32, device = device )
# c_variational = torch.tensor( 0.0, dtype = torch.float32, device = device )
