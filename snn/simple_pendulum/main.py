...
#%% ---------------------------------------- SPIKING SIMPLE PENDULUM PINN EXAMPLE MAIN SCRIPT ----------------------------------------

# This file serves to implement the main code necessary to integrate the Yuan-Li PDE for a dynamical system representing a simple pendulum using the Physics Informed Neural Network (PINN) framework.
...
flow_function2 = lambda s: torch.unsqueeze( torch.unsqueeze( -1.417322835*s[ :, 2, -1 ] + -73.575*torch.sin( s[ :, 1, -1 ] ), dim = 1 ), dim = 2 )              # [-] Flow function associated with the second state of the underlying dynamical system.
