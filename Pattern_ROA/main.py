...
# This file serves to implement the main code necessary to integrate the Yuan-Li PDE using the Physics Informed Neural Network (PINN) framework given a patterned ROA.
...
save_path = r'./Pattern_ROA/Save'                                                                    # [-] Relative path to the directory in which to save network data, figures, etc.
...
load_path = r'./Pattern_ROA/Load'                                                                    # [-] Relative path to the directory from which to load network data.
...
A0 = torch.tensor( 2, dtype = torch.float32, device = device )
S0 = torch.tensor( 20, dtype = torch.float32, device = device )
P0_shift = torch.tensor( [ 0, 0 ], dtype = torch.float32, device = device )
...
# c_monotonicity = torch.tensor( 50.0, dtype = torch.float32, device = device )               # [-] Monotonicity loss weight.
...
# c_variational = torch.tensor( 0, dtype = torch.float32, device = device )                 # [-] Variational loss weight.
